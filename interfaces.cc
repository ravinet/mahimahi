    ...
    : addresses_in_use()
{
    ...
    if ( header_line.find( "Iface" ) != 0 ) {
        throw Exception( annotate_exception( "/proc/net/route" ), "unknown format" );
    }
    ...
    for ( string line; getline( all_routes_stream, line ); ) {
        ...
        if ( dest_start == string::npos or dest_end == string::npos ) {
            throw Exception( annotate_exception( "/proc/net/route line" ), "unknown format" );
        }
        ...
        if ( len != 8 ) {
            throw Exception( annotate_exception( "/proc/net/route destination address" ), "unknown format" );
        }
...
pair< Address, uint16_t > Interfaces::first_unassigned_address( uint16_t last_octet ) const
{
    ...
    throw Exception( annotate_exception( "Interfaces" ), "could not find free interface address" );
}
