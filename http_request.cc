...
void HTTPRequest::calculate_expected_body_size( void )
{
    ...
    if ( first_line_.substr( 0, 4 ) == "GET "
        ...
        if ( !has_header( "Content-Length" ) ) {
            throw Exception( annotate_exception( "HTTPRequest" ), "does not support chunked requests" );
        }

        set_expected_body_size( true, myatoi( get_header_value( "Content-Length" ) ) );
    } else {
        throw Exception( annotate_exception( "Cannot handle HTTP method" ), first_line_ );
    }
}
...
{
    throw Exception( annotate_exception( "HTTPRequest" ), "got EOF in middle of body" );
}
