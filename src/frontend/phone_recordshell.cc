...
int main( int argc, char *argv[] )
{
    try {
        ...
        if ( argc < 2 ) {
            throw runtime_error( "Usage: " + string( argv[ 0 ] ) + " directory squid_port [command...]" );
        }
        ...
        // DNAT http_dnat( http_proxy.tcp_listener().local_address(), 80 );
        // DNAT https_dnat( http_proxy.tcp_listener().local_address(), 443 );
        // DNAT openvpn( Address(ingress_addr.ip(), openvpn_port), "UDP", openvpn_port );
        DNAT squid( Address(ingress_addr.ip(), squid_port), squid_port );
        ...
        /* Fork */
        {
            ...
            ChildProcess container_process( "recordshell", [&]() {
                    ...
                    // command.push_back("&");
                    // command.push_back( squid_proxy.command() );
