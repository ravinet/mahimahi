                                   ...
                                   const string::const_iterator & end )
{
    ...
    if ( bytes_written < 0 ) {
        throw Exception( annotate_exception( "write" ) );
    } else if ( bytes_written == 0 ) {
        throw Exception( annotate_exception( "write returned 0" ) );
    }
...
string readall( const int fd, const size_t limit )
{
    ...
    if ( bytes_read == 0 ) {
        ...
        throw Exception( annotate_exception( "read" ) );
    } else {
        /* successful read */
...
{
    if ( str.empty() ) {
        throw Exception( annotate_exception( "Invalid integer string" ), "empty" );
    }
    ...
    if ( errno != 0 ) {
        throw Exception( annotate_exception( "strtol" ) );
    } else if ( end != str.c_str() + str.size() ) {
        throw Exception( annotate_exception( "Invalid integer" ), str );
    }
