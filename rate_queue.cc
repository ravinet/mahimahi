...
RateQueue::RateQueue( const std::string & filename )
    : next_delivery_( 0 ),
      schedule_(),
...
RateQueue::QueuedPacket::QueuedPacket( const std::string & s_contents )
    : bytes_to_transmit( s_contents.size() ),
      contents( s_contents )
{}

void RateQueue::read_packet( const string & contents )
{
    packet_queue_.emplace( contents );
}

uint64_t RateQueue::next_delivery_time( void ) const
{
    return schedule_.at( next_delivery_ ) + base_timestamp_;
}

void RateQueue::use_a_delivery_opportunity( void )
{
...
void RateQueue::write_packets( FileDescriptor & fd )
{
...
int RateQueue::wait_time( void ) const
{
