...
#include "remote_proxy_server.hh"
...
int main( int argc, char *argv[] )
{
    try {
        ...
        /* Start Remote Proxy Web Servers on Egress Veth interface (http and https) */
        Address http_remote_proxy( egress_addr.ip(), 80 );
        //Address https_remote_proxy( egress_addr.ip(), 443 );
        RemoteProxyServer( http_remote_proxy, user );
        //RemoteProxyServer( https_remote_proxy, user );

        /* set up dnat */
        //DNAT dnat( egress_addr, egress_name );
        ...
        ChildProcess container_process( [&]() {
                ...
                ChildProcess bash_process( [&]() {
                        ...
                        prepend_shell_prefix( "[remoteproxy] " );
                ...
                return eventloop( move( dns_inside ), move( bash_process ) );
        ...
        ChildProcess recordr_process( [&]() {
                ...
                return eventloop( move( dns_outside ), {} );
            } );
        ...
        return eventloop( nullptr, move( child_processes ) );
               ...
               ChildProcess && child_process )
{
    ...
    return eventloop( move( dns_proxy ), move( children ) );
}
