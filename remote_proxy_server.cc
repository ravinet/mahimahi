...
RemoteProxyServer::RemoteProxyServer( const Address & addr, const string & user )
    : config_file_(),
      error_log_( "error" ),
      access_log_( "access" ),
      moved_away_( false )
{
    config_file_.write( apache_remote_proxy_config );
...
RemoteProxyServer::~RemoteProxyServer()
{
    if ( not moved_away_ ) {
...
RemoteProxyServer::RemoteProxyServer( RemoteProxyServer && other )
