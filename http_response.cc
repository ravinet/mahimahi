...
string HTTPResponse::status_code( void ) const
{
    ...
    if ( tokens.size() < 3 ) {
        throw Exception( annotate_exception( "HTTPResponse" ), "Invalid status line: " + first_line_ );
    }
...
void HTTPResponse::calculate_expected_body_size( void )
{
    ...
    if ( status_code().at( 0 ) == '1'
        ...
        /* Rule 4 */
        set_expected_body_size( false );
        throw Exception( annotate_exception( "HTTPResponse" ), "unsupported multipart/byteranges without Content-Length" );
    } else {
        /* Rule 5 */
        set_expected_body_size( false );
...
bool HTTPResponse::eof_in_body( void )
{
    ...
    if ( body_parser_ ) {
        ...
        throw Exception( annotate_exception( "HTTPResponse" ), "got EOF in middle of body" );
    }
}
