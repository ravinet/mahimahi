...
void HTTPMessage::eof( void )
{
    ...
    case HEADERS_PENDING:
        throw Exception( annotate_exception( "HTTPMessage" ), "EOF received in middle of headers" );
    case BODY_PENDING:
        if ( eof_in_body() ) {
            state_ = COMPLETE;
        }
...
const string & HTTPMessage::get_header_value( const std::string & header_name ) const
{
    ...
    throw Exception( annotate_exception( "HTTPMessage header not found" ), header_name );
}
