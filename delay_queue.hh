...
public:
    DelayQueue( const uint64_t & s_delay_ms ) : delay_ms_( s_delay_ms ), packet_queue_(), delay_map() {
        ...
        if ( dfile ) {
            ...
            while ( getline( ifile, curr_line ) ) {
                std::string ip = curr_line.substr(0, curr_line.find(" "));
                float time = stof(curr_line.substr(curr_line.find(" ")));
                delay_map[ip] = time;
                //std::cout << "ADDED ip: " << ip << " with time: " << time << std::endl;
            }
            ifile.close();
        }
    }
